    ...
    $(document).ready(async () => {
        ...
        user.data.pokemons.forEach(async (pokemon, index) => {
            let {
                types,
                stats
            } = await pokeapi('pokemon', pokemon.id);
            let species = await pokeapi('pokemon-species', pokemon.id);

            let health = parseInt(Math.trunc((2 * parseInt(stats[0].base_stat) + parseInt(
                pokemon.health_iv) + (parseInt(pokemon.health_ev) / 4)) + 100 + 10));
                    ...
                    response.log.forEach((rowLog, i) => {
                        setTimeout(() => {
                            if (i > 0) {
                                // Increment turn
                                ++turn;
                                $('.battle-log').scrollTop($(
                                    '.battle-log .turn:last'
                                ).position().top);
                            }
                            ...
                            if (turn === response.log.length) {
                                // Check if last turn
                                $('.battle-log').append(
                                    `<div class="turn" turn-order="${turn}"><div class="turn-header">Match result</div></div>`
                                );

                                // Append LOG lines
                                let lines = '';

                                rowLog[0].strings.forEach(
                                    string => {
                                        lines +=
                                            `<span>${string.replace('GYM_LEADER', '<?= $leaderName ?>')}</span>`;
                                    });

                                $(`.battle-log .turn[turn-order=${turn}]`)
                                    .append(
                                ...
                                rowLog[0].strings[0].indexOf(
                                        'GYM_LEADER') === 0 ?
                                    selector = '.user-team' :
                                    selector = '.leader-team';

                                $(`${selector} .party .party-pokemon.active`)
                                    .removeClass('active').addClass(
                                        'faint');
                                $(`${selector} .pokemons .pokemon.active`)
                                    .removeClass('active');
                            } else {
                                // Not last turn
                                $('.battle-log').append(
                                    `<div class="turn" turn-order="${turn}"><div class="turn-header">Turn ${turn}</div></div>`
                                );
                                                ...
                                                (() => {
                                                        if (heal[
                                                            ...
                                                            let currentHealth =
                                                                parseInt(
                                                                    $(
                                                                        `${selector} .pokemons .pokemon[pokemon-id=${parseInt(heal[i - 1].pokemon)}] .health .total .var`
                                                                        )
                                                                    .text()
                                                                );
                                                            let maxHealth =
                                                                parseInt(
                                                                    $(
                                                                        `${selector} .pokemons .pokemon[pokemon-id=${parseInt(heal[i - 1].pokemon)}] .health .total .max`
                                                                        )
                                                                    .text()
                                                                );
                                                            let newHealth =
                                                            ...
                                                            $(`${selector} .pokemons .pokemon[pokemon-id=${parseInt(heal[i - 1].pokemon)}] .health .var`)
                                                                .text(
                                                                    newHealth
                                                                );
                                                            $(`${selector} .pokemons .pokemon[pokemon-id=${parseInt(heal[i - 1].pokemon)}] .health-bar .bar`)
                                                                .width(
                                                                    `${barWidth}%`
                                                                );
                                                        }
                                                        ...
                                                        if (damage[
                                                            ...
                                                            let currentHealth =
                                                                parseInt(
                                                                    $(
                                                                        `${selector} .pokemons .pokemon[pokemon-id=${parseInt(damage[i - 1].pokemon)}] .health .total .var`
                                                                        )
                                                                    .text()
                                                                );
                                                            let maxHealth =
                                                                parseInt(
                                                                    $(
                                                                        `${selector} .pokemons .pokemon[pokemon-id=${parseInt(damage[i - 1].pokemon)}] .health .total .max`
                                                                        )
                                                                    .text()
                                                                );
                                                            let newHealth =
                                                            ...
                                                            setTimeout
                                                                (() => {
                                                                        if (newHealth >
                                                                            ...
                                                                            $(`${selector} .pokemons .pokemon[pokemon-id=${parseInt(damage[i - 1].pokemon)}] .sprite img`)
                                                                                .effect(
                                                                                    'bounce', {
                                                                                        distance: 10
                                                                                    },
                                                                        ...
                                                                        $(`${selector} .pokemons .pokemon[pokemon-id=${parseInt(damage[i - 1].pokemon)}] .health .var`)
                                                                            .text(
                                                                                newHealth
                                                                            );
                                                                        $(`${selector} .pokemons .pokemon[pokemon-id=${parseInt(damage[i - 1].pokemon)}] .health-bar .bar`)
                                                                            .width(
                                                                                `${barWidth}%`
                                                                            );
                                                                    },
                                                        ...
                                                        if (switched &&
                                                            ...
                                                            $(`${selector} .pokemons .pokemon[pokemon-id=${parseInt(switched)}]`)
                                                                .addClass(
                                                                    'active'
                                                                );
                                                            ...
                                                            $(`${selector} .party .party-pokemon[pokemon-id=${parseInt(switched)}]`)
                                                                .addClass(
                                                                    'active'
                                                                );
                                                        }
                                                        ...
                                                        if (
                                                            ...
                                                            setTimeout
                                                                (() => {
                                                                        $(`${selector} .party .party-pokemon[pokemon-id=${faintedPokemon}]`)
                                                                            .addClass(
                                                                                'faint'
                                                                            );
                            ...
                            $('.battle-log').scrollTop($(
                                '.battle-log .turn:last .half:last span:last'
                            ).position().top);
                        }, i * 6000);
                    });
                }
